...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        limit = options['limit']
        status = sync_mfb_hubspot_users(limit)
        if len(status['completed']) > 0:
            self.stdout.write(
                self.style.SUCCESS('Successfully synced %s users.' % len(status['completed'])))
            for message in status['completed']:
                self.stdout.write(
                    self.style.SUCCESS('Successfully synced user %s with contact %s.' % (message[0], message[1])))
        if len(status['failed']) > 0:
            self.stdout.write(
                self.style.WARNING('Failed to sync %s users.' % len(status['failed'])))
...
def sync_mfb_hubspot_users(limit):
    if limit > 10:
    ...
    # for now, we only sync users to hubspot who have signed up for a
    # communications and given TCPA consent
    ...
    for user in unsynced:
        if processed < limit:
            ...
            if hubspot_id:
                replace_pii_with_hubspot_id(hubspot_id, user)
...
def replace_pii_with_hubspot_id(hubspot_id, user):
