...
class ConnectForHealth(ProgramCalculator):
    percent_of_fpl = 4
    dependencies = ['insurance', 'income_amount',
                    'income_frequency', 'county', 'household_size']
    ...
    def eligible(self) -> Eligibility:
        ...
        e.member_eligibility(
            ...
            [
                (
                    lambda m: not m.insurance.has_insurance_types(
                        ('va', 'private')),
        ...
        income_band = int(fpl[self.screen.household_size] /
                          12 * ConnectForHealth.percent_of_fpl)
        gross_income = int(
            self.screen.calc_gross_income('yearly', ('all',)) / 12)
...
class CFHCache(Cache):
    ...
    def update(self):
        ...
        data = {d[0].strip() + ' County': float(d[1].replace(',', ''))
                for d in sheet_values}
