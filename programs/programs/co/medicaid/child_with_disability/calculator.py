    ...
    def eligible(self) -> Eligibility:
        ...
        for benefit in self.data:
            if benefit["name_abbreviated"] == "medicaid":
                ...
                break
        e.condition(
            not is_medicaid_eligible, messages.must_not_have_benefit("Medicaid")
        )

        fpl = self.program.fpl.as_dict()
        income_limit = (
            fpl[self.screen.household_size]
            * MedicaidChildWithDisability.max_income_percent
        )
        earned = max(
            0,
            int(
                self.screen.calc_gross_income("yearly", ["earned"])
                - MedicaidChildWithDisability.earned_deduction
            ),
        )
        ...
        e.member_eligibility(
            ...
            [
                (
                    lambda m: m.age <= MedicaidChildWithDisability.max_age,
                    messages.child(),
                ),
                (
                    lambda m: m.long_term_disability or m.visually_impaired,
                    messages.has_disability(),
                ),
                (
                    lambda m: m.insurance.has_insurance_types(
                        MedicaidChildWithDisability.insurance_types
                    ),
                    None,
                ),
                (
                    lambda m: not (
                        m.calc_gross_income("yearly", ["earned"]) >= 0 and m.age >= 16
                    ),
                    None,
                ),
            ],
