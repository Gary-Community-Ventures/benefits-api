...
class Command(BaseCommand):
    ...
    def eligibility(self, screen):
        ...
        for benefit, calculator in calculators.items():
            raw_result = calculator(
                screen,
                [
                    {
                        "name_abbreviated": "medicaid",
                        "eligible": False if benefit == "cfhc" else True,
                    },
    ...
    def handle(self, *args, **options):
        ...
        for directory in os.listdir(base_path):
            ...
            screen = self.create_screen(
                os.path.join(base_path, directory, "screen.yaml")
            )
            eligibility = self.eligibility(screen)
            self.update_history(
                eligibility, os.path.join(base_path, directory, "history.yaml"), date
            )
