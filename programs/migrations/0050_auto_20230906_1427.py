    ...
    for urgent_need in UrgentNeed.objects.all():
        ...
        for field in translated_fields:
            translation = Translation.objects.add_translation(
                f"urgent_need.{main_name}_{urgent_need.id}-{field}",
                getattr(urgent_need, field),
            )
...
def navigator_migrations(apps, _):
    ...
    for navigator in Navigator.objects.all():
        ...
        for field in translated_fields:
            translation = Translation.objects.add_translation(
                f"navigator.{main_name}_{navigator.id}-{field}",
...
class Migration(migrations.Migration):
    dependencies = [
        (
            "programs",
            "0049_navigator_assistance_link_1_navigator_description_1_and_more",
        ),
