...
def generate_bwf_snapshots():
    ...
    screens = Screen.objects.filter(external_id__in=bwf_ids).order_by(
        "-submission_date"
    )
    ...
    print(
        "Total BWF screens without snapshots found: "
...
def add_from_json(new_json_str):
    ...
    screen = Screen.objects.create(
        **{
            ...
            if k not in ("household_members", "id", "uuid", "user", "expenses")
        },
    )
    ...
    for member in new_json["household_members"]:
        household_member = {
            ...
            if k not in ("income_streams", "expenses", "screen", "id")
        }
        ...
        for income in member["income_streams"]:
            income = {
                ...
                if k not in ("household_member", "screen", "id")
            }
            incomes.append(
                IncomeStream(**income, screen=screen, household_member=member_model)
            )
    for expense in new_json["expenses"]:
        ...
        expenses.append(
            Expense(**expense, screen=screen, household_member=member_model)
        )
