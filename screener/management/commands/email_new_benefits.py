    ...
    def handle(self, *args, **options):
        ...
        for screen in screens:
            try:
                previous_snapshot = EligibilitySnapshot.objects.filter(
                    is_batch=True, screen=screen
                ).latest("submission_date")
            except ObjectDoesNotExist:
                self.stdout.write(
                    self.style.WARNING(
                        f"No snapshots for screen with id of {screen.id}"
                    )
                )
                continue
        ...
        for snapshot in latest_snapshots:
            ...
            existing_users.append(
                hubspot.format_email_new_benefit(
                    snapshot.screen.user, num_eligible, value
                )
            )
        ...
        if not len(existing_users):
            self.stdout.write(
                self.style.WARNING(
                    "No users in HubSpot. Make sure that you add users to HubSpot first"
                )
            )
            return
