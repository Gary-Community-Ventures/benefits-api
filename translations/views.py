...
class NewTranslationForm(forms.Form):
    ...
    default_message = forms.CharField(
        widget=forms.Textarea(attrs={"name": "text", "rows": 3, "cols": 50})
    )
...
def admin_view(request):
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            response.headers[
                "HX-Redirect"
            ] = f"/api/translations/admin/{translation.id}"
            return response
...
def filter_view(request):
    translations = Translation.objects.filter(
        label__contains=request.GET.get("label", "")
...
def translation_view(request, id=0):
    if request.method == "GET":
        ...
        translations = {
            t.language_code: TranslationForm({"text": t.text})
...
def edit_translation(request, id=0, lang="en-us"):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            forms = {
                t.language_code: TranslationForm({"text": t.text})
                for t in parent.translations.all()
            }
            context = {
                "translation": parent,
                "langs": forms,
...
def programs_view(request):
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            response.headers[
                "HX-Redirect"
            ] = f"/api/translations/admin/programs/{program.id}"
            return response
...
def navigators_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            response.headers[
                "HX-Redirect"
            ] = f"/api/translations/admin/navigators/{navigator.id}"
            return response
...
def urgent_needs_view(request):
    if request.method == "GET":
        ...
        context = {"urgent_needs": urgent_needs}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            response.headers[
                "HX-Redirect"
            ] = f"/api/translations/admin/urgent_needs/{urgent_need.id}"
            return response
...
def documents_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            response.headers[
                "HX-Redirect"
            ] = f"/api/translations/admin/documents/{document.id}"
            return response
