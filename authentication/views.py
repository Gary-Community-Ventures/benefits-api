...
from integrations.services.hubspot.integration import (
    ...
    def update(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            if (
                ...
                and not screen.is_test_data
                and not settings.DEBUG
            ):
                update_send_offers_hubspot(
                    user.external_id, user.send_offers, user.send_updates
                )
            else:
...
def upsert_user_to_hubspot(screen, user):
    ...
    if user is None or screen.is_test_data is None:
        return
    should_upsert_user = (
        (user.send_offers or user.send_updates)
        and user.external_id is None
        and user.tcpa_consent
    )
